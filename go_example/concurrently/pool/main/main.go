// pool 패키지를 이용하여 데이터 베이스 연결 풀을 생성하고 활용하는 예제
package main
...
import (
	...
	"github.com/onlynewro/documents/go_example/concurrently/pool"
)

const (
	maxGoroutines   = 25 // 실행할 수 있는 고루틴의 최대 개수
	pooledResources = 2  // 풀이 관리할 리소수의 개수
)
...
func main() {
	...
	// 풀에서 데이터 베이스 연결을 가져와 질의를 실행한다.
	for query := 0; query < maxGoroutines; query++ {
		//각 고루틴에 질의 값의 복사본을 전달해야 한다.
